...
public:
	...
	{
		return "Andy's Bi-Exponential Smooth uses just a single control: Radius.  The Radius collects the colors and then using the Euclidean Color Distance smooths similar colors within that radius.  The output is similar to a separable Bilateral with edges preserved nicely.  Note: This is purely experimental, haven't found anything similar to any other edge preserving methods or research papers.";
	}
	...
	int GetUIParameters (UIParameters* pParameters)
	{
		...
		AddParameter( PARAM_RADIUS ,"Radius", 10.0, 1.0, 100.0, TYPE_SLIDER, 0.0);
...
#pragma region		//blurs horizonally
		for(int y = 0; y < nHeight; y++)
		{
			for(int x = 0; x < nWidth; x++)
			{
				...
				float sumWeight = 0;
				unsigned int nIdx1 = (y * nWidth + x) * 4;
				...
				for(int i = x - radius; i <= x + radius; i++)
				{
					if (i < 0 || i > widthminus) continue;
					{
						...
						float imagedistance = expf((imageDist / radius) * (imageDist / radius) * 1.0f);
						
						float colordistance = expf(8.0f * colorDist / 1);
						float currWeight = 0.5f / (imagedistance * colordistance);
				...
				{//output
					//(y * nWidth + x) * 4;
					horizonaltemp[nIdx1 + CHANNEL_R] = sumred;
					horizonaltemp[nIdx1 + CHANNEL_G] = sumgreen;
					horizonaltemp[nIdx1 + CHANNEL_B] = sumblue;
				}//end output
			}
		}
#pragma endregion
...
#pragma region		//blurs vertically
		for(int x = 0; x < nWidth; x++)
		{
			for(int y = 0; y < nHeight; y++)
			{
				...
				float centerred = horizonaltemp[nIdx1 + CHANNEL_R];
				float centergreen = horizonaltemp[nIdx1 + CHANNEL_G];
				float centerblue = horizonaltemp[nIdx1 + CHANNEL_B];
				...
				{//output
					temp[nIdx1 + CHANNEL_R] = sumred;
					temp[nIdx1 + CHANNEL_G] = sumgreen;
					temp[nIdx1 + CHANNEL_B] = sumblue;
				}//end output
			}
		}
#pragma endregion
